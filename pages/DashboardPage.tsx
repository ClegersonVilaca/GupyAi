...
const DashboardPage: React.FC = () => {
        ...
        const checkUserStatus = async () => {
            ...
            if (user) {
                const { data: profile } = await supabase
                    ...
                    .select('is_pro_user')
                    .eq('id', user.id)
                    .maybeSingle(); // Better for cases where profile might not exist yet
    ...
    const handleOptimize = async () => {
        ...
        try {
            ...
            alert('Houve um erro na análise. Verifique se o Supabase Edge Function está rodando.');
        } finally {
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-full hover:bg-gray-50 transition-colors border border-transparent hover:border-border-light group">
                            <div className="text-right hidden sm:block">
                                <p className="text-sm font-bold leading-none text-text-main group-hover:text-primary transition-colors">Ana Silva</p>
                            ...
                            <div className="bg-primary/10 rounded-full size-9 flex items-center justify-center overflow-hidden border border-gray-100 ring-2 ring-transparent group-hover:ring-primary/20 transition-all">
                                <img alt="Profile picture of a woman smiling" className="w-full h-full object-cover" src="https://picsum.photos/id/65/40/40" />
                            </div>
                        </button>
                    </div>
                </header>
